...
def build_conviction(payload: SynthesisPayload, *, storage: Storage, run_dir: str) -> ConvictionArtifact:
    ...
    downside = max(price.value - scenario_values["bear"].value, 0.01)
    upside = max(scenario_values["bull"].value - price.value, 0.0)
    ratio = upside / downside
    score_value = _score_from_ratio(ratio)
    ...
    sizing = SizingInputs(
        ...
        up_down_ratio=_computed_number(
            ...
            derivation=f"inputs: {run_dir}/scenarios.json bull and bear values; {run_dir}/handoff.json price",
        ),
    ...
    artifact = ConvictionArtifact(
        ...
        conviction_score=_computed_number(
            ...
            source_name="D-2 Conviction",
            produced_at=produced_at,
            derivation="inputs: up/down ratio, filed Senior decision package completeness, and filed risk/edge artifacts",
        ),
        horizon=horizon,
        ...
        confidence_and_gaps={
            ...
            "couldnt_verify": [] if payload.method_directive.method == "DCF" else ["full non-DCF valuation model is deferred"],
