...
7. D-3 must file the final pydantic Handoff artifact under the run directory.
8. The current NO-GO path must remain unchanged.
9. Existing M0-M3 artifact paths must remain stable.
10. Existing Senior gate and ratify call counts must remain unchanged.
11. M4b must add no new Senior touchpoint.
12. M4b must not require live LLM access.
...
- The final Handoff must include:
  ...
  - risk sheet;
  - edge statement;
  - sizing inputs when available from existing artifacts;
  - confidence and gaps;
